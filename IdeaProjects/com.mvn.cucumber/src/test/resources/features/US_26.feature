Feature: The functions of the course_form/course_edit media section should work correctly
  @1
  Scenario: The functions of the course_form/course_edit media section should work correctly
    Given The desired option should be searchable from the Course overview provider dropdown
    Given Relevant url should be entered in Course overview url textbox
    Given Image can be uploaded to Course thumbnail
